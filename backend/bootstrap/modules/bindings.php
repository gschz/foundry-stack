    ...
    try {
        if (! $application->bound('cache')) {
            $application->singleton(
                'cache',
                fn (Application $app): CacheManager => new CacheManager($app)
            );
        }
        ...
        if (! $application->bound('translator')) {
            $application->singleton(
                ...
                function (Application $app): Illuminate\Translation\Translator {
                    ...
                    $locale = 'en';
                    if ($app->has('config')) {
                        $localeValue = $app->make(Repository::class)->get('app.locale');
                    ...
                    $fallback = 'en';
                    if ($app->has('config')) {
                        $fallbackValue = $app->make(Repository::class)->get('app.fallback_locale');
